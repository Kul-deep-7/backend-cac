...
const updateUserAvatar = asyncHandler(async(req,res)=>{
    ...
    if(!localAvatar){
        throw new ApiError("Local Avatar file not found")
    }

    const avatar = await uploadOnCloudinary(localAvatar)

    if(!avatar.url){
        throw new ApiError("Error while uploading avatar on cloudinary")
    }
...
const updateCoverImage = asyncHandler(async(req,res)=>{
    ...
    if (!localCoverImage) {
        throw new ApiError("local image file not found")
    }

    const coverImage = await uploadOnCloudinary(localCoverImage)

    if (!coverImage.url) {
        throw new ApiError("cover Image not uploaded on cloudinary")
    }
